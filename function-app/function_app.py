...
def normalize_payload(body: dict) -> Optional[str]:
    ...
    if isinstance(body.get("alert"), dict):
        ...
        if is_ddl:
            ddl_clause = (
                f"\n\n████ ESTE INCIDENTE ES UN CONTROL DE CAMBIO/SOX, NO UN CHEQUEO DE "
                f"SALUD ████\nSe ejecuto un comando DDL ({ddl_cmd or 'CREATE/ALTER/DROP'}) "
                f"sobre el objeto '{ddl_obj}' por el usuario '{ddl_user}' en la BD de "
                f"TALENTO. Que el SQL Server este 'Ready'/sano es IRRELEVANTE — NO es el "
                f"punto. La UNICA pregunta es: ¿este DDL estaba AUTORIZADO por una ventana "
                f"de cambio? Procedimiento OBLIGATORIO:\n"
                f"  1) Llama lookup_changes(ci_id='CI-SQL-V2', estado='aprobado').\n"
                f"  2) Revisa si el objeto '{ddl_obj}' coincide con el objeto_patron de "
                f"ALGUNA CHG aprobada (p.ej. 'dbo.nomina_*' cubre 'dbo.nomina_x' pero NO "
                f"'dbo.tabla_fantasma').\n"
                f"  3a) Si NINGUNA CHG cubre el objeto → es un CAMBIO NO AUTORIZADO. "
                f"OBLIGATORIO en report_incident: categoria='base_datos', "
                f"requiere_remediacion=true, playbook='talento-sql-revoke-ddl', "
                f"confianza_pct>=85. NO lo marques 'informativo'. La causa_raiz debe decir "
                f"que '{ddl_obj}' se creo/modifico fuera de toda ventana de cambio aprobada.\n"
                f"  3b) Solo si una CHG SI lo cubre → requiere_remediacion=false, citando "
                f"el chg_id que lo autoriza.\n"
                f"No te distraigas con el estado operativo del servidor; resuelve la "
                f"pregunta de autorizacion y actua en consecuencia."
            )
        ...
        if is_dam:
            dam_clause = (
                f"\n\n████ ESTE INCIDENTE ES DE SEGURIDAD (ACCESO INDEBIDO A BD), NO UN "
                f"CHEQUEO DE SALUD ████\nAcceso sospechoso a la BD de TALENTO (usuario "
                f"'{dam_user or 'desconocido'}', origen '{dam_ip or 'desconocido'}'). Que "
                f"la BD este sana es IRRELEVANTE. Correlaciona: (1) "
                ...
                f"(3) lookup_cmdb(ref='sqlserver-ecopetrol2') para el CI/owner. Salvo que "
                f"el acceso sea claramente legitimo, trata esto como acceso indebido: "
                f"OBLIGATORIO en report_incident categoria='seguridad', "
